...
#Study Area
This study took place at the Census Subdivision level to target the municipality of Vancouver, BC. There are 
```{r}
...
# returning census data and geography for the Vancouver CSD as an sf-class dataframe
...
The point data for parks, street trees, and homeless shelters in the Vancouver CSD was obtained from csv files from Vancouver's open data portal and transformed into data frames with columns describing the X and Y coordinates of each location.
```{r}
ggplot(census_data) +
...
# using ggplot to create a cartogram for low income status households in census tracts for the Vancouver CSD
ggplot(CT_pop_cartogram) +
  geom_sf(aes(fill = cut_number(census_data$Proportion.low.income, 5)), colour = "white", size = 0.1) +
  scale_fill_brewer(palete = "YlOrRd") +
  labs(fill = "Proportion of Low Income Status")
```
