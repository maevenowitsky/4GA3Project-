...
```{r}
census_data <- cbind(census_data, access$accessibility)
```
...
ggplot(census_data) +
geom_sf(aes(fill = cut_number(census_data$acessibility, 5)),
        color = "black",
        size = 0.1) +
scale_fill_brewer(palette = "YlOrRd") +
...
model_population <- lm(formula = population_2021 ~ access, 
             data = census_data)
...
ggplot(data = census_data, 
  ...
  geom_smooth(formula = y ~ x,
              method = "lm") +
  xlab("Population (2021)") +
  ylab("Number of Parks Accessible within 30 Minutes (Through walking or public transit") 
```
...
model_population_density <- lm(formula = population-density ~ access, 
             data = census_data)
...
```{r}
#creating a regression model of population density regressing on number of accessible parks
model_visible_minority <- lm(formula = Proportion_visible_minority ~ access, 
             data = census_data)

stargazer(model_visible_minority,
          header = FALSE,
          title = "Percent Visible Minority Population in Census Tracts regressed on Number of Parks Accessible")
```
Scatterplot of proportion of visible minorites vs number of accessible parks
```{r}
ggplot(data = census_data, 
       aes(x = access, 
  ...
  geom_smooth(formula = y ~ x,
              method = "lm") +
  xlab("Visible Minority Population (%)") +
  ylab("Number of Parks Accessible within 30 Minutes (Through walking or public transit") 
```
...
```{r}
#creating a regression model of population density regressing on number of accessible parks
model_nonvisible_minority <- lm(formula = Proportion_not_a_visible_minority ~ access, 
             data = census_data)

stargazer(model_nonvisible_minority,
          header = FALSE,
          title = "Percent Non-Visible Minority Population in Census Tracts regressed on Number of Parks Accessible")
```
Scatterplot of proportion of non-visible minorities vs number of accessible parks
```{r}
ggplot(data = census_data, 
       aes(x = access, 
           y = Proportion_not_a_visible_minority))+
  geom_point() +
  geom_smooth(formula = y ~ x,
              method = "lm") +
  xlab("Non-Visible Minority Population (%)") +
  ylab("Number of Parks Accessible within 30 Minutes (Through walking or public transit") 
```
...
```{r}
#creating a regression model of population density regressing on number of accessible parks
model_income <- lm(formula = Proportion_not_a_visible_minority ~ access, 
             data = census_data)
